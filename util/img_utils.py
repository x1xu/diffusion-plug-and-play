...
class mask_generator:
    ...
    def __call__(self, img):
        if self.reuse_mask:
            mask = torch.load(self.mask_path)
            return mask
        ...
        if self.mask_path != '':
            torch.save(mask, self.mask_path)
...
def adjust_color(img, model, tol):
    # Choose the best image among the ambiguities of phase retrieval according to scores.
    t = torch.abs(2.0 + 0. * tol).float()
    scale = 1.0
    rescaled_img = img * scale
    candidate = img.detach().clone()
    badness_prev = torch.inf
    for inverted_colors in product([False, True], repeat=3):
        new_img = rescaled_img.detach().clone()
        for i_color, is_inverted in enumerate(inverted_colors):
            if is_inverted:
                new_img[..., i_color, :, :] *= -1
        pred_eps = model(new_img, t)
        badness = torch.sum(pred_eps ** 2)
        if badness < badness_prev:
            badness_prev = badness
            candidate = new_img / scale
    # For an unknown reason the FFHQ model prefers green-inverted image
    candidate[..., 1, :, :] *= -1   # Invert the green channel back
    # candidate[..., 0, :, :] *= -1   # Sometimes helpful
